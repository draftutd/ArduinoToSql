...
<title>Sensorthree</title>
<?PHP
$page = $_GET['Page'];
$rows=60;
$total_data = mysql_num_rows(mysql_query("SELECT * FROM sensorthree"));
$total_page=ceil($total_data/$rows);
$start=(($page-1)*$rows); 
$objQuery = mysql_query("SELECT * FROM sensorthree LIMIT $start,60") or die ("Error Query [SELECTFROM sensorthree]");
$objQuerychart = mysql_query("SELECT * FROM sensorthree ");
    ...
    function drawChart() {
		...
		<? for ( $i = 0; $i < $total_data ; $i++) {
			while($rowCharts = mysql_fetch_array($objQuerychart)) {
				?>
         data.addRow(['<?=$rowCharts["Time"]?>',<?=$rowCharts["Sensordatathree"]?>]);
		<? }}?>
...
<center><h1>กราฟแสดงผลการบันทึกค่าการตรวจจับเซนเซอร์ตัวที่สาม</h1><div id="chart_div" style="height: 300px; width: 100%;"></div></center>
...
<div class="container" >
     ...
     while($row = mysql_fetch_array($objQuery))
 {
         ...
         echo "<td><center>" .$row["ID"] .  "</center></td> ";
		  echo "<td><center>" .$row["Date"] .  "</center></td> ";
		   ...
		   echo "<td><center>" .$row["Sensordatathree"] .  "</center></td> ";
 ...
 <nav aria-label="Page navigation example">
  <ul class="pagination">
    ...
    <li <? if($page==$i){echo 'class="active"';}else {echo 'class="page-item"';}?>><a class="page-link" href="sensorthree.php?Page=<?=$i?>"><?=$i?></a></li>
    <?}?>
